...
export default function useBUSDPrice(currency?: Currency): Price | undefined {
  ...
  const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
    () => [
      [chainId && wrapped && currencyEquals(WRO, wrapped) ? undefined : currency, chainId ? WRO : undefined],
      [wrapped?.equals(BUSD_MAINNET) ? undefined : wrapped, chainId === ChainId.MAINNET ? BUSD_MAINNET : undefined],
      [chainId ? WRO : undefined, chainId === ChainId.MAINNET ? BUSD_MAINNET : undefined],
    ],
  ...
  return useMemo(() => {
    ...
    // handle weth/eth
    if (wrapped.equals(WRO)) {
      if (busdPair) {
        const price = busdPair.priceOf(WRO)
    ...
    const ethPairETHAmount = ethPair?.reserveOf(WRO)
    const ethPairETHBUSDValue: JSBI =
      ethPairETHAmount && busdEthPair ? busdEthPair.priceOf(WRO).quote(ethPairETHAmount).raw : JSBI.BigInt(0)
    ...
    if (ethPairState === PairState.EXISTS && ethPair && busdEthPairState === PairState.EXISTS && busdEthPair) {
      if (busdEthPair.reserveOf(BUSD_MAINNET).greaterThan('0') && ethPair.reserveOf(WRO).greaterThan('0')) {
        const ethBusdPrice = busdEthPair.priceOf(BUSD_MAINNET)
        const currencyEthPrice = ethPair.priceOf(WRO)
