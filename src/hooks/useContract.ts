...
import {
  ...
  getNFTCollectibleContract,
  getNFTCollectionManagerContract
...
export const useNFTCollectibleContract = (address: string) => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getNFTCollectibleContract(address, library.getSigner()), [address, library])
}
export const useNFTCollectionManagerContract = () => {
  const { account, library } = useActiveWeb3React()
  return useMemo(() => getNFTCollectionManagerContract(account? library.getSigner() : library),[library, account])
}
