...
const deserializeFarm = (farm: SerializedFarm): DeserializedFarm => {
  const { lpAddresses, lpSymbol, tag, pid, dual, multiplier, isCommunity, quoteTokenPriceBusd, tokenPriceBusd } = farm
...
export const usePollCoreFarmData = () => {
  ...
  useEffect(() => {
    dispatch(fetchFarmsPublicDataAsync([2, 8, 9, 19, 24, 26, 27, 28, 30, 31, 32, 33, 34, 35, 36, 37, 38, 40, 41, 42, 43, 44, 45, 46, 47]))
...
export const usePriceCakeBusd = (): BigNumber => {
  ...
  let cakePriceBusdAsString = null
  if (cakePriceUsd) {
    cakePriceBusdAsString = 0
  }
