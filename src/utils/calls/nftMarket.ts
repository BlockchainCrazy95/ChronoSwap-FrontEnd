 ...
 * @returns transaction hash, or null
 */
export const buyTokenUsingWCRO = async (
  contract: ethers.Contract,
  collectionAddress: string,
  tokenId: number,
  ...
  try {
    const tx = await contract.buyTokenUsingWCRO(collectionAddress, tokenId, price)
    const receipt = await tx.wait()
    return receipt.transactionHash
  } catch (error) {
    console.error(error)
...
/**
 * Get pending WCRO revenues for a user
 * @param contract
 * @param userAddress
...
/**
 * Claim WCRO pending revenues for a connected users
 * @param contract
 * @returns transaction hash, or null
 */
