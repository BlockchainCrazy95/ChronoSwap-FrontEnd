...
import { useFarms, useBusdPriceFromPid } from 'state/farms/hooks'
...
const useGetTopFarmsByApr = (isIntersecting: boolean) => {
  ...
  const cakePriceBusd = useBusdPriceFromPid(2)
  ...
  useEffect(() => {
    const getTopFarmsByApr = (farmsState: DeserializedFarm[]) => {
      ...
      globalThis.tvlFarms = 0
      const excludedFarms = [1, 3, 4, 5, 6, 7, 10, 11, 12, 13, 14, 15, 16, 17, 18, 20, 21, 22, 23]
      sortedByApr.forEach((farm) => {
        if (!excludedFarms.includes(farm.pid)) {
          globalThis.tvlFarms += Math.round(farm.lpTotalInQuoteToken.times(farm.quoteTokenPriceBusd).toNumber())
        }
      })
