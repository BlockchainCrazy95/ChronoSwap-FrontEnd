...
const useGetTopPoolsByApr = (isIntersecting: boolean) => {
  ...
  useEffect(() => {
    const getTopPoolsByApr = (activePools: DeserializedPool[]) => {
      ...
      globalThis.tvlPools = 0
      sortedByApr.forEach((pool) => {
        if (!pool.isAutoVault) {
          globalThis.tvlPools += Math.round(
            (pool.totalStaked.toNumber() / 1000000000000000000) * pool.stakingTokenPrice,
          )
        }
      })
      setTopPools(sortedByApr)
    }
