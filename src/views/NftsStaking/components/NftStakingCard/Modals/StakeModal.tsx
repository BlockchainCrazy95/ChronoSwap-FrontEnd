...
const StakeModal: React.FC<StakeModalProps> = ({
  ...
  const { sousId, stakingToken, earningTokenPrice, userData, stakingLimit, earningToken } = pool
  const { t } = useTranslation()
  const { theme } = useTheme()
  const { onStake } = useStakeNft(sousId, isBnbPool)
  const { onUnstake } = useUnstakeNft(sousId, pool.enableEmergencyWithdraw)
