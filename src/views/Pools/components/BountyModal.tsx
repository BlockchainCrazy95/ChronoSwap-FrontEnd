...
const BountyModal: React.FC<BountyModalProps> = ({ onDismiss, TooltipComponent }) => {
  ...
  const handleConfirmClick = async () => {
    setPendingTx(true)
    try {
      const tx = await callWithGasPrice(cakeVaultContract, 'harvest', undefined, { gasLimit: 420000 })
      const receipt = await tx.wait()
      if (receipt.status) {
        toastSuccess(
          t('Bounty collected!'),
          <ToastDescriptionWithTx txHash={receipt.transactionHash}>
            {t('CNO bounty has been sent to your wallet.')}
    ...
    <Modal title={t('Claim Bounty')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <Flex alignItems="flex-start" justifyContent="space-between">
        ...
        <Flex flexDirection="column">
          <Balance bold value={cakeBountyToDisplay} decimals={7} unit=" CNO" />
      ...
      <Flex alignItems="center" justifyContent="space-between">
        ...
        <Balance color="textSubtle" value={totalYieldToDisplay} unit=" CNO" />
      </Flex>
